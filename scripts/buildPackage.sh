...
BUILD_ARTIFACT="terraform-provider-modtm"
...
function release() {
  ...
  for os_arch in "${OS_ARCH[@]}" ; do
    ...
    (
      env GOOS="${OS}" GOARCH="${ARCH}" CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X 'github.com/Azure/terraform-provider-modtm/version.ProviderVersion=v${VERSION}'" -o "${BUILD_ARTIFACT}_${OS}_${ARCH}${EXT}"
